...
def resetar_formulario():
    ...
    for key in list(st.session_state.keys()):
        if any(key.startswith(prefix) for prefix in ["f_", "n_", "c_", "t_sol", "is_novo"]):
    ...
    with st.container(border=True):
        ...
        if st.button("Entrar", use_container_width=True):
            if cargo_sel == "Reserva de Cesta Básica" and is_sistema_bloqueado():
                st.markdown(f"<div class='bloqueio-msg'><h3>🚫 ACESSO BLOQUEADO</h3><p>O prazo de reservas expirou.</p></div>", unsafe_allow_html=True)
    ...
    # --- VISÃO: RESERVA ---
        ...
        if st.button("💾 SALVAR TUDO", type="primary", use_container_width=True):
            ...
            try:
                ...
                st.balloons()
                st.success("✅ RESERVAS SALVAS!"); time.sleep(1.5); resetar_formulario(); st.rerun()
            except Exception as e: st.error(f"Erro: {e}")
