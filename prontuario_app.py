...
if not st.session_state.autenticado:
            ...
            def exibir_registro_compacto(item):
                with st.container(border=True):
                    ...
                    with c1:
                        ...
                        st.caption(f"📍 Retirada: {item.get('local_retirada')} | 📅 {item.get('data_sistema')}")
                    with c2:
                    ...
                    with c3:
                        ...
                        st.caption(f"Congregação: {item.get('comum_solicitante')}")
                    with c4:
    ...
    # --- VISÃO DA IRMÃ ---
        ...
        with st.container(border=True):
            ...
            st.markdown("### 2. Dados do Assistido / Prontuário")
            col_p1, col_p2, col_p3 = st.columns([1.5, 1.5, 1])
            n_pront = col_p1.text_input("Número do Prontuário (se houver):", key=f"p_{f_id}")
            comum_ast = col_p2.text_input("Comum do Assistido:", key=f"com_a_{f_id}")
            q_cestas = col_p3.number_input("Cestas:", min_value=1, value=1, key=f"q_{f_id}")
            
            loc = st.radio("Local de Retirada:", ["Pq. Guarani", "Itaquera"], horizontal=True, key=f"l_{f_id}")
            
            st.divider()
            
            is_novo = st.toggle("É UM CADASTRO NOVO? (Preencher endereço)", key=f"v_{f_id}")
            ...
            if st.button("💾 FINALIZAR E SALVAR", type="primary", use_container_width=True):
                ...
                if not nome_sol or not comum_sol or not comum_ast: 
                    st.error("Preencha o Nome e as Congregações (Solicitante e Assistido)!"); st.stop()
                ...
                payload = {
                    ...
                    "comum_solicitante": comum_sol,
                    "comum_assistido": comum_ast,
                    "num_prontuario": n_pront, 
