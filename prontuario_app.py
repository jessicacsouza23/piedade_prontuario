...
from datetime import datetime, timedelta  # Importação correta e completa
import pandas as pd
import numpy as np
import time
import pytz
...
def resetar_formulario():
    ...
    for key in list(st.session_state.keys()):
        if any(key.startswith(p) for p in ["f_", "n_", "c_", "ts_", "inv_", "np_", "qp_", "nc_", "ca_", "id_", "ba_", "civ_", "nco_", "ico_", "bco_", "en_", "bai_", "cep_", "qn_", "loc_", "ns_", "cs_"]):
    ...
    # --- VISÃO: RESERVA ---
        ...
        if st.button("💾 ENVIAR RESERVA", type="primary", use_container_width=True):
            ...
            try:
                ...
                st.balloons(); st.success("✅ RESERVA ENVIADA!"); time.sleep(1); resetar_formulario(); st.rerun()
